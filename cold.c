...
(
    ...
    char *out_path,            /* I:   output_path                   */
    int METHOD,                 /* I:   CCD or SCCD                    */
...
//                    }
    ...
    if (bl_train == 1)
    {
        ...
        rec_cg[*num_curve].change_prob = (int)((double)(adj_conse - id_last) * 100.0 / (double)adj_conse) ;
...

    {
        ...
        if ((end - i_start + 1) >= CONSE_END) //04/02/2019 change adj_conse to CONSE_END
        {
            /******************************************************/
            /*                                                    */
         ...
         if ((end - i_start + 1) >= CONSE_END)     // 09/28/2018 SY delete equal sign //11/15/2018 put back equal sign
         {
            for (i_b = 0; i_b < TOTAL_IMAGE_BANDS; i_b++)
            {
//                    for(k=i_start-1;k<end;k++)
