...
probability = 0.95
min_days_conse = 80
row = 1000 # please change as needed
col = 1000 # please change as needed
method = 2 # 1 is COLD; 2 is S-CCD
user_mask_path = ''
output_mode = 11 # 11: both output; 1: output states; 10: output observation file
...
ccd_plot = np.fromfile(out_path + '/record_change_' + str(row) + '_' + str(col) + '_sccd.dat', dtype=sccd_dt)

# read state records
plot_state = pd.read_csv(out_path + '/StateRecords_' + str(row) + '_' + str(col) + '_' + bn_shortname + '.csv', na_values=-9999)
